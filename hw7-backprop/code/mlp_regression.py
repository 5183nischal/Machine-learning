...
class MLPRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 5000):
        ...
        self.prediction = nodes.VectorScalarAffineNode(x=self.hid2, w=self.W2, b=self.b2,
    ...
    def fit(self, X, y):
        ...
        init_values = dict() ## TODO
        print(self.init_param_scale)
        init_values['W1'] = self.init_param_scale*(2*np.random.rand(self.num_hidden_units, num_ftrs)-1)
        init_values['b1'] = self.init_param_scale*(2*np.random.rand(self.num_hidden_units)-1)
        init_values['w2'] = self.init_param_scale*(2*np.random.rand(self.num_hidden_units)-1)
        init_values['b2'] = self.init_param_scale*(2*np.random.rand(1)-1)

        #print(init_values['W1'], init_values['b1'])
