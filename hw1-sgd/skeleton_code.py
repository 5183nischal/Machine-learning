...
def feature_normalization_nugget(x):        #The new normalization that takes one array as input
    ...
    for i in range(0, x.shape[1]):
        if not std[i] == 0:  #Ensures features with constant vales don't collapse
...
def feature_normalization(train, test):
    ...
    #Defined a new function that takes one array at a time as input called feature_normalization_nugget
...
def grad_checker(X, y, theta, epsilon=0.000001, tolerance=1e-4):
    ...
    for i in range(num_features):
        val = (compute_square_loss(X, y, theta + identity[:,i]) - compute_square_loss(X, y, theta - identity[:,i]))/2*epsilon
        approx_grad[i] = val
...
def batch_grad_descent(X, y, alpha=0.05, num_iter=1000, check_gradient=False):
    ...
    for i in range (num_iter):
        loss_hist[i] = compute_square_loss(X,y,theta)       #Loss at the theta value stored
        theta = theta - alpha*compute_square_loss_gradient(X,y,theta)       #Update theta against the gradient
...
def compute_regularized_square_loss_gradient(X, y, theta, lambda_reg):
    ...
    grad = ((2*(Xnorm.dot(theta) - Xtranspose.dot(y)))/m) + 2*lambda_reg*theta        #Gardient at theta

    #print(Xnorm.shape, theta.shape, Xtranspose.shape, (Xtranspose.dot(y)).shape, grad.shape)
...
def regularized_grad_descent(X, y,  alpha=0.03, lambda_reg = 0.02245, num_iter=1000):
    ...
    for i in range (num_iter):
        loss_hist[i] = compute_square_loss(X,y,theta) + lambda_reg*regularization       #Loss at the theta value stored
        theta = theta - alpha*compute_regularized_square_loss_gradient(X,y,theta, lambda_reg)       #Update theta against the gradient
        theta_hist[i,:] = theta           #Theta history updates
...
def stochastic_grad_descent(X, y, alpha=0.003, lambda_reg=0.02245, num_iter=1000):
    ...
    for i in range (num_iter):
        ...
        regularization = (theta.transpose()).dot(theta)
        loss_hist[i] = compute_square_loss(inputx,inputy,theta) + lambda_reg*regularization       #Loss at the theta value stored
        ...
        theta = theta - alpha*grad[1,:]       #Update theta against the gradient
